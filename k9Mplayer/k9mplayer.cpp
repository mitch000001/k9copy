...
//    m_xineWidget->slotSetDvdDevice( _device);
    m_device=_device;
}
...
/*
   int ipos=buf.find("ANS_PERCENT_POSITION=");
   if (ipos>=0 && !m_seeking) {
	QString sPosition=buf.mid(ipos).section("=",1,1);
        qDebug(buf);

	m_position=sPosition.toInt();
	slider->setValue(m_position);
   }
*/
...
//	slider->setEnabled(m_xineWidget->isSeekable());
//	slider->setValue(m_xineWidget->getPosition());
}
...
void K9Mplayer::open( k9DVD *_dvd,k9DVDTitle *_title,int chapter) {
	...
	cbAudio->changeItem(aud->getlanguage(),aud->getID()-1);
    }
	...
	cbSub->changeItem(sub->getlanguage(),sub->getID()-1);
    }
...
//  int currentTitle=m_xineWidget->currentDVDTitleNumber();
//  if (m_title!=currentTitle)
//      m_xineWidget->slotStop();

}

void K9Mplayer::cbAudioActivated( int _value) {
 //   m_xineWidget->slotSetAudioChannel( _value +1 );
...
void K9Mplayer::cbSubActivated( int _value) {
 //  m_xineWidget->slotSetSubtitleChannel( _value +1);
